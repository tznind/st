// Animal Companion loyalty tracking functionality
(function() {
  'use strict';
  
  console.log('Animal Companion loyalty script loading...');
  
  function initializeAnimalCompanionLoyalty() {
    console.log('Initializing animal companion loyalty...');
    
    const loyaltyShapes = document.querySelectorAll('.animal-companion-card [data-track-id="ac_loyalty"]');
    const loyaltyLabel = document.querySelector('.animal-companion-card .track-label');
    ...
    let currentLoyalty = parseInt(urlParams.get('ac_loyalty')) || 0;
    ...
    function updateURL(value) {
      ...
      if (value > 0) {
        params.set('ac_loyalty', value.toString());
      } else {
        params.delete('ac_loyalty');
      }
    ...
    loyaltyShapes.forEach((shape, index) => {
      if (!shape.hasAttribute('data-ac-loyalty-listener')) {
        ...
        shape.setAttribute('data-ac-loyalty-listener', 'true');
      } else {
    ...
    console.log('Animal Companion loyalty initialization complete');
  }
  
  // Create global initialization function that can be called whenever card is recreated
  window.initializeAnimalCompanion = function() {
    console.log('Initializing Animal Companion card...');
    initializeAnimalCompanionLoyalty();
  };
  
  // Multiple initialization attempts for first load
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initializeAnimalCompanionLoyalty);
  } else {
    initializeAnimalCompanionLoyalty();
  }
  
  // Also try after a delay in case cards are loaded dynamically
  setTimeout(initializeAnimalCompanionLoyalty, 500);
  setTimeout(initializeAnimalCompanionLoyalty, 1000);
})();
